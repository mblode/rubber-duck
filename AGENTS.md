...
- Voice tool calls (`read_file`, `write_file`, `edit_file`, `bash`, `grep_search`, `find_files`, `web_search`) are executed by `cli/src/daemon/voice-tools.ts` via the `voice_tool_call` daemon method. Swift no longer implements these tools locally. **Seven tools, not six.** `web_search` was missing from this list for a while and the marketing site inherited the error. `apps/web/lib/tools.test.ts` now parses the dispatcher's `case` arms and fails if the published tool table and the switch disagree, so adding an eighth tool breaks the web build until the table is updated too.
