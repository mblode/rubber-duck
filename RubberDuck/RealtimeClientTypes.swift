...
enum RealtimeErrorRetryDisposition: String {
    case retryable
    case nonRetryable
}

struct RealtimeErrorClassification {
    let type: String
    let code: String
    let message: String
    let param: String?
...
func classifyRealtimeErrorPayload(_ payload: [String: Any]) -> RealtimeErrorClassification {
    ...
    if nonRetryableTypes.contains(type)
        ...
        // Bias toward retry for unknown classes to avoid dropping recoverable sessions.
        disposition = .retryable
    }
