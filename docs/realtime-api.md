...
Install the Agents SDK (requires Zod v4)
...
await session.connect({ apiKey: 'ek\_...(put your own key here)' });
...
// curl -s -X POST https://api.openai.com/v1/realtime/client_secrets -H "Authorization: Bearer $OPENAI*API_KEY" -H "Content-Type: application/json" -d '{"session": {"type": "realtime", "model": "gpt-realtime"}}' | jq .value
